...
def train_step(model: torch.nn.Module, 
    ...
    for batch, (X, y) in enumerate(dataloader):
        ...
        with torch.autocast(device_type=device, enabled=True):
            ...
            loss_classification = loss_fn_classification(class_out, y_cumulative)
        ...
        y_pred_class = torch.sum(torch.round(torch.sigmoid(class_out)), dim=1, keepdim=True).squeeze(dim=1) - 1
